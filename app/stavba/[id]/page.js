...
//
// CHANGELOG:
// 20260328_08    – export do PDF (jsPDF) a Excel (SheetJS): tlačítka v záložce Rozbor
...
export default function StavbaPage() {
  ...
  const toggleRef = useRef(toggleTheme) // vždy aktuální toggle funkce
  useEffect(() => { toggleRef.current = toggleTheme }, [toggleTheme])
      ...
      const parsedEBC = {
        ...
        zemni: {
          ...
          mat_vlastni:  [{ id:uid(), popis:'Materiál vlastní', castka:String(Math.round(matVlastniCelkem * 100) / 100) },
                         ...(subRows['X SUB 01'] ? [{ id:uid(), popis:'Materiál vlastní X SUB 01', castka:String(Math.round(subRows['X SUB 01'])) }] : [])],
        ...
        dof: {
          ...
          popl_ver_prostranstvi: { rows:[{ id:uid(), popis:'Poplatky za veřejné prostranství',   castka:String(gnRowAll(['1102003','1102003_'])) }], open:false },
  ...
  const applySazby = async (sazby) => {
    ...
    const updated = {
      ...
      import_build: `20260328_08 / ${String(now.getDate()).padStart(2,'0')}.${String(now.getMonth()+1).padStart(2,'0')}.${now.getFullYear()} ${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}:${String(now.getSeconds()).padStart(2,'0')}`,
    }
    setS(updated)
        ...
        <div style={{ maxWidth: tab==='rozbor' ? '100%' : 1060, margin:'0 auto', padding: tab==='rozbor' ? '0 120px' : '0' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 0 2px', flexWrap:'wrap' }}>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize:10, color:T.muted, letterSpacing:1.5, textTransform:'uppercase', display:'flex', gap:12, alignItems:'center' }}><span>Rozbor staveb · {s.oblast}</span>{tab==='vstup' && <span style={{ color:'#64748b', fontFamily:'monospace' }}>📦 20260328_08</span>}</div>
          ...
          <div className="no-print" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:6, marginBottom:4 }}>
            ...
            <div style={{ display:'flex', gap:8, flexShrink:0, alignItems:'center' }}>
              {tab === 'rozbor' ? (<>
                ...
                <button onClick={toggleTheme} style={{ padding:'5px 10px', background:'transparent', border:`1px solid ${T.border}`, borderRadius:6, fontSize:16, cursor:'pointer', lineHeight:1 }}>{dark ? '☀️' : '🌙'}</button>
              </>) : (<>
                {canEdit && (
                  <button onClick={() => setRozpisDialog(true)}
                    style={{ padding:'6px 12px', background:'rgba(16,185,129,0.15)', border:'1px solid rgba(16,185,129,0.4)', borderRadius:6, color:'#10b981', fontSize:12, fontWeight:700, cursor:'pointer', whiteSpace:'nowrap' }}>
